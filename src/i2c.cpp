...
esp_err_t i2c_read(uint8_t device_address, uint8_t reg_addr, uint8_t *data, size_t len) {
    return i2c_master_write_read_device(_i2c_port, device_address, &reg_addr, 1, data, len,
...
esp_err_t i2c_write_byte(uint8_t device_address, uint8_t reg_addr, uint8_t data) {
    ...
    ret = i2c_master_write_to_device(_i2c_port, device_address, write_buf, sizeof(write_buf),
                                     I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);

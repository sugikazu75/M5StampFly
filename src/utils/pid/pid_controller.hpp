...
class PIDController {
   public:
    PIDController(const float p_gain, const float i_gain, const float d_gain, const float limit_sum = 1e6,
                  const float limit_p = 1e6, const float limit_i = 1e6, const float limit_d = 1e6,
                  const float limit_err_p = 1e6, const float limit_err_i = 1e6, const float limit_err_d = 1e6)
        : result_(0), err_p_(0), err_i_(0), err_i_prev_(0), err_d_(0), p_term_(0), i_term_(0), d_term_(0) {
    ...
    void setPGain(const float p_gain) {
        p_gain_ = p_gain;
    }
    void setIGain(const float i_gain) {
        i_gain_ = i_gain;
    }
    void setDGain(const float d_gain) {
        d_gain_ = d_gain;
    }
    ...
    void setLimitSum(const float limit_sum) {
        limit_sum_ = limit_sum;
    }
    void setLimitP(const float limit_p) {
        limit_p_ = limit_p;
    }
    void setLimitI(const float limit_i) {
        limit_i_ = limit_i;
    }
    void setLimitD(const float limit_d) {
        limit_d_ = limit_d;
    }
    void setLimitErrP(const float limit_err_p) {
        limit_err_p_ = limit_err_p;
    }
    void setLimitErrI(const float limit_err_i) {
        limit_err_i_ = limit_err_i;
    }
    void setLimitErrD(const float limit_err_d) {
        limit_err_d_ = limit_err_d;
    }

    void setLimits(const float limit_sum, const float limit_p, const float limit_i, const float limit_d,
                   const float limit_err_p, const float limit_err_i, const float limit_err_d) {

...
Motor::Motor(int motor_pin, int motor_channel, BLA::Matrix<3, 1> pos, BLA::Matrix<3, 1> axis, float sigma)
    : motor_pin_(motor_pin), motor_channel_(motor_channel), sigma_(sigma) {
    for (int i = 0; i < 3; i++) {
        pos_(i)  = pos(i);
        axis_(i) = axis(i);
    }

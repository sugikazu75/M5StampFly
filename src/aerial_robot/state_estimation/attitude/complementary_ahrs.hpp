...
class ComplementaryAHRS {
   public:
    ...
    void setAcc(BLA::Matrix<3, 1> acc) {
        acc_ = acc;
    }
    void setGyro(BLA::Matrix<3, 1> gyro) {
        gyro_ = gyro;
    }
    void setMag(BLA::Matrix<3, 1> mag) {
        mag_ = mag;
    }
    ...
    BLA::Matrix<3, 1> getMag() {
        return mag_;
    }
    BLA::Matrix<3, 1> getRpy() {
        return rpy_;
    }
    BLA::Matrix<3, 1> getEstG() {
        return est_g_;
    }
    BLA::Matrix<3, 1> getEstM() {
        return est_m_;
    }
    ...
    void estimation() {
        ...
        /* acc correction */
        if (valid_acc == 1 && cnt == 0) est_g_ = (est_g_tmp * GYR_CMPF_FACTOR + acc_) * INV_GYR_CMPF_FACTOR;
        ...
        rpy_(2) = atan2f(est_m_(2) * est_g_(1) - est_m_(1) * est_g_(2),
                         est_m_(0) * invG * sq_g_y_sq_g_z -
                             (est_m_(1) * est_g_(1) + est_m_(2) * est_g_(2)) * invG * est_g_(0));  // + mag_declination

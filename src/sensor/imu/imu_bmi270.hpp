...
class ImuBMI270 : public Imu {
   ...
   private:
    ...
    void imu_update(void) {
        bmi2_get_sensor_data(&imu_data_, pBmi270_);
    }
    float imu_get_acc_x(void) {
        return lsb_to_mps2(imu_data_.acc.x, 8.0, 16) / GRAVITY_EARTH;
    }
    float imu_get_acc_y(void) {
        return lsb_to_mps2(imu_data_.acc.y, 8.0, 16) / GRAVITY_EARTH;
    }
    float imu_get_acc_z(void) {
        return lsb_to_mps2(imu_data_.acc.z, 8.0, 16) / GRAVITY_EARTH;
    }
    float imu_get_gyro_x(void) {
        return lsb_to_rps(imu_data_.gyr.x, DPS20002RAD, 16);
    }
    float imu_get_gyro_y(void) {
        return lsb_to_rps(imu_data_.gyr.y, DPS20002RAD, 16);
    }
    float imu_get_gyro_z(void) {
        return lsb_to_rps(imu_data_.gyr.z, DPS20002RAD, 16);
    }
